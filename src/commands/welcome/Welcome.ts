...
export default class WelcomeCommand extends Command {
  data = new SlashCommandBuilder()
    ...
    .addSubcommand((sub) =>
      sub.setName('status').setDescription('View welcome configuration')
    )
    .addSubcommand((sub) =>
      sub
        .setName('channel')
    ...
    .addSubcommand((sub) =>
      sub.setName('reset').setDescription('Reset welcome configuration')
    )
  ...
  private async handleEnable(
    ...
    await repo.upsertConfig(guildId, { welcome_enabled: true });
    await interaction.editReply({ content: 'Welcome messages enabled.' });
  }
  ...
  private async handleDisable(
    ...
    await repo.upsertConfig(guildId, { welcome_enabled: false });
    await interaction.editReply({ content: 'Welcome messages disabled.' });
  }
  ...
  private async handleChannel(
    ...
    await repo.upsertConfig(guildId, { welcome_channel_id: channel.id });
    await interaction.editReply({
  ...
  private async handleMessage(
    ...
    await repo.upsertConfig(guildId, { welcome_message: message });
    await interaction.editReply({ content: 'Welcome message updated.' });
  }
  ...
  private async handleEmbed(
    ...
    await repo.upsertConfig(guildId, { welcome_use_embed: enabled });
    await interaction.editReply({
      content: `Welcome embed ${enabled ? 'enabled' : 'disabled'}.`,
    });
  }
  ...
  private async handleReset(
    ...
    await interaction.editReply({ content: 'Welcome configuration reset.' });
  }
}
